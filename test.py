...
from database.database import start_polling_loop,update_slack_response
...
def handle_approve_button(ack, body, client):
    ...
    update_slack_response(int(request_reference), "approved", button_clicked_time)
...
def handle_reject_button(ack, body, client):
    ...
    email = get_user_email(user_id)
    button_clicked_time = datetime.now(pytz.timezone('US/Eastern')).strftime('%m-%d-%Y - %I:%M %p EST')
    update_slack_response(int(request_reference),"rejected",button_clicked_time)
    client.chat_update(
        channel=channel_id,
        ts=original_message,
